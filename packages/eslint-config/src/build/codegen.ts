...
export function generateConfigModule(opts: ConfigOptions): string {
  ...
  // Build inline helper blocks (e.g. react-compat plugin)
  const helpers: string[] = []
  if (hasReact) {
    helpers.push(...emitReactCompatHelper())
  }
...
function emitReactCompatHelper(): string[] {
  return [
    ...
    '  const dom = eslintReactPlugin.configs.dom.plugins["@eslint-react/dom"].rules',
    '  const webApi = eslintReactPlugin.configs["web-api"].plugins["@eslint-react/web-api"].rules',
    '  const hooksExtra = eslintReactPlugin.configs.recommended.plugins["@eslint-react/hooks-extra"].rules',
    '  const naming = eslintReactPlugin.configs.recommended.plugins["@eslint-react/naming-convention"].rules',
    '  const rsc = eslintReactPlugin.configs.rsc.plugins["@eslint-react/rsc"].rules',
    "",
    "  // Legacy aliases: legacy name → [source, original name]",
    "  // Only semantically equivalent mappings belong here.",
    "  const aliases = {",
    '    "jsx-boolean-value": [core, "jsx-shorthand-boolean"],',
    '    "jsx-fragments": [core, "jsx-shorthand-fragment"],',
    ...
    '    "jsx-no-useless-fragment": [core, "no-useless-fragment"],',
    ...
    '    "prefer-read-only-props": [core, "prefer-read-only-props"],',
    ...
    '    "hook-use-state": [naming, "use-state"],',
    '    "jsx-pascal-case": [naming, "component-name"],',
    ...
    "  const identicalCore = [",
    '    "no-access-state-in-setstate", "no-array-index-key", "no-children-prop",',
    '    "no-direct-mutation-state", "no-string-refs", "no-redundant-should-component-update",',
    '    "no-unused-class-component-members", "no-unused-state",',
    '    "jsx-no-comment-textnodes", "jsx-no-duplicate-props", "jsx-no-undef",',
    '    "jsx-uses-react", "jsx-uses-vars",',
    "  ]",
    '  for (const n of identicalCore) aliases[n] = [core, n]',
    "",
    "  // Build aliased originals set",
    "  const aliasedCore = new Set(Object.values(aliases).filter(([s]) => s === core).map(([, n]) => n))",
    "  const rules = {}",
    "",
    ...
    "  const subs = [[dom, new Set(['prefer-namespace-import'])], [webApi], [hooksExtra], [naming], [rsc]]",
