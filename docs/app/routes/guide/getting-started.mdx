...
If you want the full workflow instead of wiring files by hand, use the CLI:

```bash
npx eslint-config-setup init
```

That starts the interactive setup wizard.
...
- `eslint.config.ts`
- `oxlint.config.ts` when `--oxlint` is enabled
- package scripts such as `lint`, `check`, `fix`, and formatter scripts
- `AGENTS.md` for AI coding assistants
- `.vscode/settings.json`
- `.githooks/pre-commit` or `.husky/pre-commit`

By default, `init` will not overwrite conflicting config files, scripts, or companion files. Use `--dry-run` to preview changes, and `--force` only when you intentionally want to replace an existing generated target.

See [CLI Workflow](/guide/cli-workflow) for the command reference.
