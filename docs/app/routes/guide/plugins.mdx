...
| **React** | `strict-type-checked` | Based on @eslint-react's strictest type-aware preset. 80+ rules across core, DOM, Web API, hooks, naming, and RSC — unified under a single `react/` namespace via the [React Compat Plugin](/adr/0020-react-compat-plugin) for OxLint compatibility. |
...
|--------|-------------|---------|
| [`@eslint-react/eslint-plugin`](https://eslint-react.xyz/) | `react: true` | 80+ React rules (core, DOM, Web API, hooks, naming, RSC) under unified `react/` namespace — [31 rules run in OxLint](/adr/0020-react-compat-plugin) |
| [`eslint-plugin-react-hooks`](https://www.npmjs.com/package/eslint-plugin-react-hooks) | `react: true` | Rules of Hooks and exhaustive dependency checking |
