...
The overlapping React surface is split by namespace:

- `react/*` rules with legacy-compatible names can shift to OxLint.
- `jsx-a11y/*` rules shift when OxLint supports the same accessibility check.
...
As OxLint expands React, Hooks, JSX-a11y, and React Perf coverage, the rule-surface guardrail in CI forces us to review the new overlap before config behavior drifts.
