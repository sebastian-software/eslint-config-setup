...
- **`react/no-forward-ref`** — Use ref as a prop instead of `forwardRef`
...
The **React Compat Plugin** bridges this gap. It registers semantically compatible @eslint-react rules under the legacy names that OxLint expects, while keeping genuinely new semantics under their modern names. The result: `eslint-plugin-oxlint` correctly identifies which rules OxLint already handles and disables them in ESLint.
